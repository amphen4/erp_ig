...
class FacturacionAjaxController extends Controller
{
    /**
    ...
    public function filtrarFacturas(Request $request){
        ...
        if($request->inicio == 'null' && $request->fin == 'null'){
                ...
                $arreglo['facturas'] = Factura::orderBy('fecha','desc')->limit(100)->get()->toArray();
                foreach($arreglo['facturas'] as  $p){
                    $arreglo['medio_pago'][] = Factura::find($p['id'])->ot->medio_pago;
                ...
                $arreglo['facturas'] = Factura::whereBetween('fecha', [$request->inicio, $request->fin])->orderBy('fecha','desc')->limit(100)->get()->toArray();
                foreach($arreglo['facturas'] as  $p){
                    $arreglo['medio_pago'][] = Factura::find($p['id'])->ot->medio_pago;
