...
class VentasPerfilController extends Controller
{
	/**
    ...
    {
        return view('ventasuser.perfil.perfil');
    }
    /**
    ...
    public function guardarPerfil(Request $request)
    {
        $this->validate($request, ['name' => 'required|string|max:100',
                                   'email' => ['required','string','email','max:100', Rule::unique('ventasusers')->ignore(Auth::user()->id)],
        ...
        if($request->perfil)
        {
            ...
            $filename = 'ventas-'.Auth::user()->id.'.jpg';
            if($file)
            {
        ...
        return redirect(url('/ventasuser/perfil'))->with('mensaje','Perfil actualizado correctamente!');
    }
}
