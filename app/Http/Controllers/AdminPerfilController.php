...
class AdminPerfilController extends Controller
{
    ...
    {
        return view('adminuser.perfil.perfil');
    }
    /**
    ...
    public function guardarPerfil(Request $request)
    {
        $this->validate($request, ['name' => 'required|string|max:100',
                                   'email' => ['required','string','email','max:255', Rule::unique('adminusers')->ignore(Auth::user()->id)],
        ...
        if($request->perfil)
        {
            ...
            $filename = 'admin-'.Auth::user()->id.'.jpg';
            if($file)
            {
        ...
        return redirect(url('/adminuser/perfil'))->with('mensaje','Perfil actualizado correctamente!');
    }
}
