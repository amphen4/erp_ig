...
@section('title','Ordenes de Trabajo | Produccion')
@section('css')
<!-- Datatables -->
                  ...
                  <div class="x_content">
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      <tbody id="bodyTabla">
                        ...
                        <tr>
                          <td>@if($fila->otestado->nombre == 'EN PROCESO')<a href="{{ url('/produccionuser/ots').'/'.$fila->id.'/edit' }}"  class="btn btn-warning btn-xs ">Modificar</a>@endif</td>
                          <td>{{ $fila->id }}</td>
                          <td>{{ $fila->fecha }}</td>
                          ...
                          <td><a target="_blank" href="{{url('produccionuser/reporteCotizacion').'/'.$fila->cotizacion->id}}"  class="btn btn-xs"><span class="fa fa-file-pdf-o" aria-hidden="true"></span> Cotizacion</a><a target="_blank" href="{{url('produccionuser/reporteOt').'/'.$fila->id}}" class="btn btn-xs"><span class="fa fa-file-pdf-o" aria-hidden="true"></span> O.T</a></td>
...
<script>
  function aplicarFiltro(){
    ...
    $.ajax({
      type: "POST",
      url: "{{url('produccionuser/filtrarOts')}}",
      ...
      success: function(json) {
        ...
        for(var i=0; i<data['ots'].length; i++){
          ...
          var tiene = '';
          if(data['otestado'][i] == 'EN PROCESO') tiene = '<a href="'+"{{url('produccionuser/ots')}}"+'/'+data['ots'][i].id+'/edit" class="btn btn-warning btn-xs">Modificar</a>';
          wea.row.add([tiene,
                      data['ots'][i].id,
                      data['ots'][i].fecha,
                      data['otestado'][i],
                      ...
                      'Credito Nota',
                      data['adminuser'][i],
                      '<a target="_blank" href="{{url("produccionuser/reporteCotizacion")}}'+'/'+data['idcotizacion'][i]+'" class="btn btn-xs"><span class="fa fa-file-pdf-o" aria-hidden="true"></span> Cotizacion</a><a href="{{url("produccionuser/reporteOt")}}/'+data['ots'][i].id+'" class="btn btn-xs"><span class="fa fa-file-pdf-o" aria-hidden="true"></span> O.T</a>',
