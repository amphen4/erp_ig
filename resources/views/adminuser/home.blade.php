...
<!-- calendar modal -->
    ...
    <script src="{{ asset('templates/gentelella') }}/vendors/fullcalendar/dist/lang/es.js"></script>
    <script>
    	 /* CALENDARIO (wow) */
		  $(document).ready(function() { 
				...
				var calendar = $('#calendar').fullCalendar({
					...
					$(".antosubmit").on("click", function() {
					  ...
					  if (title) {
					  	 ...
					  	 $.ajax({
							...
							url: '{{url("adminuser/enviarEvento")}}',
				  ...
				  eventDrop: function(event, delta, revertFunc, jsEvent, ui, view) {
					  	...
					  	$.ajax({
							type: "POST",
							url: "{{url('adminuser/actualizarEvento')}}"+"/"+event.id,
							headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
				  ...
				  eventClick: function(calEvent, jsEvent, view) {
					...
					$(".antosubmit2").on("click", function() {
					  calEvent.title = $("#title2").val();
					  	$.ajax({
							type: "POST",
							url: "{{url('adminuser/actualizarEvento')}}"+"/"+calEvent.id,
							headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
					...
					$(".antosubmit3").on("click", function() {
					  calEvent.title = $("#title2").val();
					  	$.ajax({
							type: "POST",
							url: "{{url('adminuser/eliminarEvento')}}"+"/"+calEvent.id,
							headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
							data: 'method=DELETE',
