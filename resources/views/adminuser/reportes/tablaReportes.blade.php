                  ...
                  <div class="x_content">
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      <tbody id="bodyTabla">
                        ...
                        <tr>
                          <td><a href="{{url('adminuser/reportes/').'/'.$fila->id}}"  class="btn btn-primery btn-xs ">Ver Historial</a></td>
                          <td>{{ $fila->id }}</td>
                          <td>{{ $fila->fecha }}</td>
                          ...
                          <td><a target="_blank" href="{{url('adminuser/reporteCotizacion').'/'.$fila->cotizacion->id}}"  class="btn btn-xs"><span class="fa fa-file-pdf-o" aria-hidden="true"></span> Cotizacion</a><a target="_blank" href="{{url('adminuser/reporteOt').'/'.$fila->id}}" class="btn btn-xs"><span class="fa fa-file-pdf-o" aria-hidden="true"></span> O.T</a></td>
...
<script>
    ...
    $.ajax({
      ...
      success: function(json) {
        ...
        for(var i=0; i<data['ots'].length; i++){
          ...
          wea.row.add(['<a href="{{url('adminuser/reportes/').'/'}}'+data['ots'][i].id+'"  class="btn btn-primery btn-xs ">Ver Historial</a>',
                      data['ots'][i].id,
                      data['ots'][i].fecha,
                      data['otestado'][i],
                      ...
                      '<a target="_blank" href="{{url("adminuser/reporteCotizacion")}}'+'/'+data['idcotizacion'][i]+'" class="btn btn-xs"><span class="fa fa-file-pdf-o" aria-hidden="true"></span> Cotizacion</a><a href="{{url("adminuser/reporteOt")}}/'+data['ots'][i].id+'" class="btn btn-xs"><span class="fa fa-file-pdf-o" aria-hidden="true"></span> O.T</a>',
  ...
  $('.btnEliminar').click(function(){
    if(confirm('Esta seguro?. De todos modos quedará registro de todos sus estados y repectivos detalles en Seccion Reportes')){
      $.ajax({
        type: "POST",
        url: "{{url('adminuser/ots')}}"+'/'+$(this).val(),
        headers:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        data: '_method=DELETE',
        success: function(json) {
