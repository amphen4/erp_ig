                  ...
                  @endif
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td><a href="{{ url('/adminuser/marcas').'/'.$fila->id.'/edit' }}"  class="btn btn-warning btn-xs ">Editar</a><!--<button   class="btn btn-danger btn-xs " value="{{$fila->id}}" >Eliminar</button>--></td>
                          <td>{{ $fila->id }}</td>
                          <td>{{ $fila->nombre }}</td>
...
<div  class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form enctype="multipart/form-data" class="form-horizontal" role="form" method="POST" action="{{ url('/adminuser/marcas') }}">
...
<form id="formEliminar" method="POST" action="{{url('/adminuser/marcas/')}}" enctype="multipart/form-data">
  {{ csrf_field() }}
  {{ method_field('DELETE') }}
</form>
@endsection
...
<script>
    ...
    $(".btn-danger").click(function(){
      ...
      {
        $('#formEliminar').attr('action',"{{url('/adminuser/marcas/')}}"+'/'+$(this).val());
        $('#formEliminar').submit();
      }  
    });
